...
int main() {
    ...
    glUniformMatrix4fv(glGetUniformLocation(triangleShader.ID,"transform"),1,GL_FALSE,glm::value_ptr(trans));

    float delta = 0.0f;
    float lastFrame = 0.0f;
    int transLoc = glGetUniformLocation(triangleShader.ID, "transform");
    // Main loop
    while (!glfwWindowShouldClose(window)) {
        ...
        trans = glm::rotate(trans, glm::radians(blend_amount*1000.0f*delta),glm::vec3(0.0,0.0,1.0));
